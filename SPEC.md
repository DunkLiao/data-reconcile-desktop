...
提供一個全域的「數值誤差容許值」，套用於所有數值比較欄位。

規則：

- 留空 = 關閉，所有欄位維持嚴格字串比對。
- 輸入 `0` = 啟用數值比對，忽略前導零與格式差異（例如 `00123` = `123`、`1,000` = `1000`）。
- 逐格自動判定：只有當兩邊的值都能解析為有限數字時才套用容許值；否則回退為字串比對。
- 當兩邊皆為數字時，若絕對差 `|A - B| <= 容許值` 即視為相同。
- 數字解析接受負號、小數、科學記號與千分位逗號。
- Key 欄位不套用容許值，一律維持嚴格字串比對（見 §28）。
- 容許值必須為大於等於 0 的數字；不合法時應阻止比較並提示。
- 比較結果與 Excel Summary 應記錄本次使用的容許值（未啟用顯示 Off）。
